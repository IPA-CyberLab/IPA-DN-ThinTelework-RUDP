...
bool g_no_clipboard = false;
bool g_no_fileshare = false;
...
const TCHAR ViewerCmdLine::NO_CLIPBOARD[] = _T("noclipboard");
const TCHAR ViewerCmdLine::NO_FILESHARE[] = _T("nofileshare");
...
void ViewerCmdLine::parse()
{
  ...
  if (isPresent(NO_CLIPBOARD))
  {
	  g_no_clipboard = true;
  }

  if (isPresent(NO_FILESHARE))
  {
	  g_no_fileshare = true;
  }
