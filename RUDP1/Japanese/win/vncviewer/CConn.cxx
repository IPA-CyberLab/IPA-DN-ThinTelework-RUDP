...
CConn::refreshMenu(bool enableSysItems) {
  ...
  if (options.menuKey) {
    ...
    TCharArray tmp(_tcslen(menuKeyStr.buf) + 64);
    _stprintf(tmp.buf, _T("%s キーを送信"), menuKeyStr.buf);
...
void CConn::serverInit() {
  ...
  AppendMenu(wndmenu, MF_STRING, IDM_FULLSCREEN, _T("フルスクリーン(&F)"));
  AppendMenu(wndmenu, MF_SEPARATOR, 0, 0);
  ...
  AppendMenu(wndmenu, MF_STRING, IDM_REQUEST_REFRESH, _T("画面全体の更新(&R)"));
//  AppendMenu(wndmenu, MF_SEPARATOR, 0, 0);
