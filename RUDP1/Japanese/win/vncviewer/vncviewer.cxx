...
int WINAPI WinMain(HINSTANCE inst, HINSTANCE prevInst, char* cmdLine, int cmdShow) {
  try {
...
#ifdef _DIALOG_CAPTURE
    if (captureDialogs) {
      CConn::userConfigKey.openKey(HKEY_CURRENT_USER, _T("Software\\Thin Telework\\URDP Client\\"));
    ...
    if (!configFiles.empty() || !hosts.empty() || acceptIncoming) {
      ...
      config.start(HKEY_CURRENT_USER, _T("Software\\Thin Telework\\URDP Client\\"));

      // - Tell the rest of VNC Viewer where to write config data to
      CConn::userConfigKey.createKey(HKEY_CURRENT_USER, _T("Software\\Thin Telework\\URDP Client\\"));
